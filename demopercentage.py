...
shape_groups = {}
shapes = {
    'bardisplay': AnimBarDisplay ( Rect((100,500),(500,50)), color=(0,0,255))
}
...
def animate():
    # Add animations here
    if (frame >= kf['start'] and frame <= kf['end']):
        frame_range = (kf['end']-kf['start'])
        percent = 100 * ((frame - kf['start']) / frame_range )
        shapes['bardisplay'].percent = percent
