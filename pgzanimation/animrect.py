...
class AnimFilledRect(AnimFilledPolygon):
    ...
    def __init__(self, rect, color, anchor=('center', 'center')):
        # convert rect into points (once changed then can no longer use it as a rect as that would break rotation). But it can be replaced with a new rectangle (or by replacing the points list it will become a polygon). You can also get a rect, but if rotated then that will be bounding rectange (but could be useful for collision detection)
        points = self._points = [rect.topleft, rect.topright, rect.bottomright, rect.bottomleft]
    ...
    def rect(self, new_rect):
        self._points = rect_to_points(new_rect)
